...
/**
 * Handler for removing a user account.
 */
class UserAccountsDeleteHandlerSpec extends Specification {
    UserDAO userDAO
    ...
    def "deletes a user account"() {
        when:
        def userToDelete = userDAO.listAll().first()
        delete("esa/user_accounts/${userToDelete.id}")

        then:
        with(response.body) {
            userDAO.listAll().size() == 1
            !userDAO.listAll()*.id.contains(userToDelete.id)
        }
    }
}
